...
# enable argcomplete for this script only
sudo apt install register-python-argcomplete3
eval "$(register-python-argcomplete3 ios_toolkit)"
...
def main():
    ...
    upload_parser.add_argument('-v', '--verbose', help='enable verbose', action='store_true', default=False)
    ...
    download_parser.add_argument('-v', '--verbose', help='enable verbose', action='store_true', default=False)
    ...
    backup_parser.add_argument('-v', '--verbose', help='enable verbose', action='store_true', default=False)
    ...
    try:
        ...
        # backup device
            ...
            for dev in args.device:
                # print(dev)
                # exit(1)
                backup(dev, verbose=args.verbose)
        ...
        # backup from file list
            ...
            try:
                with open(args.file, 'r') as inputfile:
                    ...
                    for router in routers:
                        backup(router, verbose=args.verbose)
            ...
            try:
                with open(args.file, 'r') as inputfile:
                    ...
                    # remove accidental blank in the list, these blanks returns all records in database.
                    routers = list(filter(None, inputfile))
                    threader(args.command, routers, verbose=args.verbose)
            except FileNotFoundError as e:
                print(f'There is an error FileNotFoundError: {e}')
            finally:
                pass
        ...
        # config device
            ...
            try:
                with open(args.file, 'r') as inputfile:
                    ...
                    routers = list(filter(None, inputfile))
                    for router in routers:
                        config(router, conf=args.config, verbose=args.verbose)
            except FileNotFoundError as e:
                print(f'There is an error FileNotFoundError: {e}')
            finally:
                pass
            ...
            try:
                with open(args.file, 'r') as inputfile:
                    ...
                    # remove accidental blank in the list, these blanks returns all records in database.
                    routers = list(filter(None, inputfile))
                    threader(args.command, routers, conf=args.config, verbose=args.verbose)
            except FileNotFoundError as e:
                print(f'There is an error FileNotFoundError: {e}')
            finally:
                pass
